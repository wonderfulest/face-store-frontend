...
const cardQuestions = [
	{
		key: "already-paid",
		icon: (
			<svg width="40" height="40" fill="none" stroke="#1db954" strokeWidth="2.5" viewBox="0 0 40 40">
				<text x="10" y="30" fontSize="28" fontWeight="bold">$</text>
			</svg>
		),
		title: "Already Paid?",
	...
	},
	{
		...
		icon: (
			<svg width="40" height="40" fill="none" stroke="#1db954" strokeWidth="2.5" viewBox="0 0 40 40">
				<circle cx="20" cy="20" r="15" stroke="#1db954" strokeWidth="2.5" fill="none"/>
				<rect x="16" y="10" width="8" height="12" rx="4" fill="none"/>
				<line x1="20" y1="20" x2="20" y2="14" stroke="#1db954" strokeWidth="2.5"/>
				<circle cx="20" cy="26" r="1.5" fill="#1db954"/>
			</svg>
		),
	...
	},
	{
		...
		icon: (
			<svg width="40" height="40" fill="none" stroke="#1db954" strokeWidth="2.5" viewBox="0 0 40 40">
				<path d="M30 20a10 10 0 1 1-10-10" stroke="#1db954" strokeWidth="2.5" fill="none"/>
				<polyline points="30,20 30,10 20,10" stroke="#1db954" strokeWidth="2.5" fill="none"/>
			</svg>
		),
		title: "Need Refund?",
	...
	},
	{
		...
		icon: (
			<svg width="40" height="40" fill="none" stroke="#1db954" strokeWidth="2.5" viewBox="0 0 40 40">
				<rect x="10" y="12" width="20" height="6" rx="2" stroke="#1db954" strokeWidth="2.5" fill="none"/>
				<rect x="10" y="22" width="20" height="6" rx="2" stroke="#1db954" strokeWidth="2.5" fill="none"/>
			</svg>
		),
...
const questionsByCategory = {
	"Installation & Purchase": [
		...
		},
		{
			q: "How do I input the Code to unlock the trial?",
			a:
				'Open the link <a href="https://pay.wristo.io/code"><strong><u>https://pay.wristo.io/code</u></strong></a>, enter the six-digit number on the watch face, complete the payment as guided, ' +
		...
		},
		{
			...
			a:
				...
				"However, if you remove and reinstall an app or clockface, it might ask for payment again. You can unlock it again for free by going to <a href=\"https://pay.wristo.io/unlock\" target=\"_blank\" rel=\"noopener noreferrer\"><strong><u>https://pay.wristo.io/unlock</u></strong></a>.",
		},
		{
			q: "How can I request a refund?",
			a:
		...
		},
		{
			q: "How can I find my purchase history?",
			a:
				"You can look up your complete purchase history at <a href=\"https://pay.wristo.io/lookup\" target=\"_blank\" rel=\"noopener noreferrer\"><strong><u>https://pay.wristo.io/lookup</u></strong></a>.",
		},
		{
			q: "I already paid! Why am I charged a second time?",
			a:
				"If you remove and reinstall an app or clockface, our system may not always recognize you and may ask you to pay again. You can unlock your clockface again for free at <a href=\"https://pay.wristo.io/unlock\" target=\"_blank\" rel=\"noopener noreferrer\"><strong><u>https://pay.wristo.io/unlock</u></strong></a> using the code shown on your watch and the email you used for the original purchase.",
		},
		{
...
const allQuestions = Object.entries(questionsByCategory).flatMap(([category, qs]) =>
	qs.map(q => ({ ...q, category }))
);
...
const Faq = () => {
	...
	const filteredQuestionsByCategory = useMemo(() => {
		if (!search.trim()) return questionsByCategory;
		const result = {};
	...
	const handleCardClick = (cardIdx) => {
		const card = cardQuestions[cardIdx];
		const qIdx = allQuestions.findIndex(q => q.q === card.q && q.category === card.category);
		setOpenIndex(qIdx);
		setTimeout(() => {
			if (questionRefs.current[qIdx]) {
				...
				<div className="faq-cards">
					{cardQuestions.map((item, idx) => (
						<button
							...
							type="button"
						>
							<div className="faq-card-icon">{item.icon}</div>
							<div className="faq-card-title">{item.title}</div>
							<div className="faq-card-desc">{item.desc}</div>
						</button>
					))}
				</div>
				<div className="faq-list">
					{Object.entries(filteredQuestionsByCategory).map(([cat, qs], catIdx) => (
						<div key={cat} className="faq-category-block">
							<h2 className="faq-category-header">{cat}</h2>
							{qs.map((item, idx) => {
								// 全局索引
								const globalIdx = allQuestions.findIndex(q => q.q === item.q && q.category === cat);
								return (
									<div className="faq-item" key={idx} ref={el => questionRefs.current[globalIdx] = el}>
										<button
											className={`faq-question ${openIndex === globalIdx ? "open" : ""}`}
										...
										>
											<span className="q-icon">{openIndex === globalIdx ? "−" : "+"}</span>
											<span dangerouslySetInnerHTML={{ __html: item.q }} />
										</button>
										<div
											className={`faq-answer ${openIndex === globalIdx ? "show" : ""}`}
											id={`faq-answer-${globalIdx}`}
											style={{ display: openIndex === globalIdx ? "block" : "none" }}
										>
